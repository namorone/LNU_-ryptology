...
class BackpackCipherApp:
    def __init__(self, master ):
        ...
        self.text_value = ""
        self.file_content = ""
        self.sequence_value = tk.StringVar(value="")
        self.m_value = tk.StringVar(value="")
        self.t_value = tk.StringVar(value="")
        self.sequence_sum = tk.StringVar(value="")
        self.master = master
        self.master.title("KnapSack Cryptosystem")
    ...
    def generate_sequence(self):
        try:
            length = int(8)
            sequence, seq_sum = generate_super_increasing_sequence(length)
            self.sequence_value = (",".join(map(str, sequence)))
            self.sequence_sum = (str(seq_sum))
            print(self.sequence_value, self.sequence_sum)
        except ValueError:
            tk.messagebox.showerror("Error", "Please enter a valid integer for the sequence length.")

    def generate_m(self):
        try:
            m = int(self.sequence_sum) + random.randint(10, 100)
            self.m_value = (str(m))
        except ValueError:
            tk.messagebox.showerror("Error", "An error occurred while generating m.")

    def generate_t(self):
        try:
            t = int(self.m_value) * random.randint(2, 10) + 1
            self.t_value = (str(t))
        except ValueError:
            tk.messagebox.showerror("Error", "Please enter a valid integer for t.")
    ...
    def button_clicked(self):
        ...
        if self.text_value == "":
            ...
            sequence = list(map(int, self.sequence_value.split(",")))
            m = int(self.m_value)
            t = int(self.t_value)
            print(sequence,m, t)

            if self.type_action == "encrypt":
                open_key = generate_open_key(sequence, m, t)
                print (open_key)
                if open_key:
                    result = encrypt(open_key, text)
                    print(result, "результат")
                else:
                    result = "Error: Unable to generate open key."
            else:
                result = decrypt(sequence, m, t, list(map(int, text.split())))
    ...
    def encrypt_file(self):
        ...
        if open_key:
            result = encrypt(open_key, self.file_content)
    ...
    def decrypt_file(self):
        ...
        result = decrypt(sequence, m, t, list(map(int, self.file_content)))
        self.file_content = result
